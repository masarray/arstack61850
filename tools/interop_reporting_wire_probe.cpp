...
void print_usage() {
    std::cout
        << "Usage: ariec61850_interop_reporting_wire_probe <host> [port] [options]\n"
        << "  --mode urcb|brcb\n"
        << "  --domain DOMAIN\n"
        << "  --rcb ITEM\n"
        << "  --expected-rptid TEXT\n"
        ...
        << "  --timeout-ms N\n\n"
        << "Replays the static external vendor external IEC 61850 client commissioning order: URCB Resv,\n"
...
int main(int argc, char** argv) {
    try {
        ...
        std::cout
            << "INTEROP_REPORTING_WIRE_PASS mode="
        ...
        std::cerr << "external IEC 61850 client reporting wire probe failed: "
                  << exception.what() << '\n';
        return 1;
    }
}
