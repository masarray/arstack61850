#!/usr/bin/env python3
"""Large-model IEDScout discovery parity regression.

Reproduces the failure shape captured from OMICRON IEDScout: a ~5k-object
SIPROTEC-like inventory where GVAA of AA1E1F06R4ESQZ1/CSWI1 must complete
...
def build_manifest() -> str:
    ...
    # Preserve the same root order seen on the golden IEDScout server.
    lines.extend([
        f"LN\t{target}\tLLN0",
        f"LN\t{target}\tCSWI1",
        f"LN\t{target}\tCILO1",
...
def main() -> int:
    ...
    port = free_port()
    with tempfile.TemporaryDirectory(prefix="arstack-iedscout-full-discovery-") as directory:
        root = Path(directory)
        ...
        with stdout_path.open("w+", encoding="utf-8") as server_out, \
            ...
            try:
                ...
                # Golden IEDScout answers the captured CSWI1 GVAA in ~61 ms.
        ...
        print(
            "IEDSCOUT_FULL_DISCOVERY_GOLDEN_PASS "
