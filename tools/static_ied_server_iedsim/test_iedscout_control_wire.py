#!/usr/bin/env python3
"""Real OMICRON IEDScout SBO-enhanced wire regression.
...
def main() -> int:
    ...
    port = free_port()
    with tempfile.TemporaryDirectory(prefix="arstack-iedscout-control-wire-") as directory:
        root = Path(directory)
        model = root / "iedscout-control.model"
        ...
        with stdout_path.open("w+", encoding="utf-8") as out, \
            ...
            try:
                ...
                if probe.returncode != 0:
                    raise RuntimeError(
                        f"IEDScout wire probe failed (exit={probe.returncode}):\n"
                        f"stdout:\n{probe.stdout}\nstderr:\n{probe.stderr}"
                    )
                if "IEDSCOUT_ZERO_CTLNUM_CONTROL_PASS" not in probe.stdout:
                    raise RuntimeError(
        ...
        if "DataAccessError=11" in server_stdout or "protocol_error" in server_stdout:
            raise RuntimeError(
                "server rejected the real IEDScout-compatible command:\n"
                + server_stdout + "\n" + server_stderr
            )
        ...
        print(
            "IEDSCOUT_REAL_CONTROL_GOLDEN_PASS "
