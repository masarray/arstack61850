...
if ($stillMissing.Count -ne 0) {
    ...
    throw "MMS server build prerequisites are still missing after checkout repair:$([Environment]::NewLine)$formatted$([Environment]::NewLine)Verify that HEAD is agent/mms-server-r1-r2-current-main and run 'git status --short'."
}

Write-Host "[OK] MMS server source preflight passed."
