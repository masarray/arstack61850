...
enum class RawMmsReceiveResult : std::uint8_t {
    none,
    frame,
    failure,
};

// Minimal passive Layer-2 receiver for TCP/102. Linux AF_PACKET and Windows
// dynamic Npcap require explicit interface selection. No client association,
// socket send, or packet injection. The parser filters TCP/102 on Linux.
class RawMmsPacketReceiver final {
public:
    RawMmsPacketReceiver() = default;
    ~RawMmsPacketReceiver() { close(); }

    RawMmsPacketReceiver(const RawMmsPacketReceiver&) = delete;
    RawMmsPacketReceiver& operator=(const RawMmsPacketReceiver&) = delete;

    RawMmsPacketReceiver(RawMmsPacketReceiver&& other) noexcept { move_from(other); }
    RawMmsPacketReceiver& operator=(RawMmsPacketReceiver&& other) noexcept {
        if (this != &other) {
            close();
            move_from(other);
        }
    ...
    [[nodiscard]] bool open(std::string_view interface_name, std::string& error) {
        ...
        if (interface_name.empty()) {
            error = "MMS monitoring requires an explicit Ethernet interface.";
            return false;
        }
#if defined(__linux__)
        ...
        if (socket_fd < 0) {
            error = "Could not open AF_PACKET raw socket for MMS monitoring: " +
        ...
        if (::bind(
            ...
            error = "Could not bind MMS monitor to " + name + ": " +
                std::string{std::strerror(errno)};
            ::close(socket_fd);
            return false;
        }

        // Port and protocol filtering is performed by the bounded decoder.
        // Do not attach the GOOSE EtherType filter to an MMS receiver.
        const int flags = ::fcntl(socket_fd, F_GETFL, 0);
        if (flags < 0 || ::fcntl(socket_fd, F_SETFL, flags | O_NONBLOCK) != 0) {
            error = "Could not make the MMS monitor socket non-blocking on " + name + ": " +
                std::string{std::strerror(errno)};
            ::close(socket_fd);
            return false;
        }
        ...
        if (wpcap_module_ == nullptr) {
            error = "Npcap wpcap.dll was not found. Install Npcap to monitor Layer-2 MMS on Windows.";
            return false;
        }
        ...
        BpfProgram program{};
        constexpr const char* filter = "tcp port 102 or (vlan and tcp port 102)";
        if (pcap_compile_(pcap_handle_, &program, filter, 1, 0xFFFFFFFFU) != 0) {
            error = "Npcap could not compile the MMS EtherType filter";
        ...
        if (filter_result != 0) {
            error = "Npcap could not install the MMS EtherType filter";
            const char* detail = pcap_geterr_(pcap_handle_);
            if (detail != nullptr && *detail != '\0') error += ": " + std::string{detail};
            close();
            return false;
...
#else
        (void)interface_name;
        error = "Raw Ethernet MMS monitoring is unavailable on this platform.";
        return false;
#endif
    }

    [[nodiscard]] RawMmsReceiveResult receive(
        ...
        if (socket_fd_ < 0 || interface_index_ <= 0) {
            error = "MMS raw Ethernet subscriber is not open.";
            return RawMmsReceiveResult::failure;
        }
        std::array<std::uint8_t, 65'535> buffer{};
        const auto received = ::recv(socket_fd_, buffer.data(), buffer.size(), 0);
        if (received < 0) {
            if (errno == EAGAIN || errno == EWOULDBLOCK) {
                return RawMmsReceiveResult::none;
            }
            error = "MMS raw Ethernet receive failed on " + interface_name_ + ": " +
                std::string{std::strerror(errno)};
            return RawMmsReceiveResult::failure;
        }
        if (received == 0) return RawMmsReceiveResult::none;
        ethernet_frame.assign(buffer.begin(), buffer.begin() + received);
        return RawMmsReceiveResult::frame;
#elif defined(_WIN32)
        if (pcap_handle_ == nullptr || pcap_next_ex_ == nullptr) {
            error = "MMS Npcap subscriber is not open.";
            return RawMmsReceiveResult::failure;
        }
        PcapPacketHeader* header{};
        const unsigned char* data{};
        const int result = pcap_next_ex_(pcap_handle_, &header, &data);
        if (result == 0) return RawMmsReceiveResult::none;
        if (result != 1 || header == nullptr || data == nullptr) {
            error = "MMS Npcap receive failed on " + interface_name_;
            const char* detail = pcap_geterr_ != nullptr ? pcap_geterr_(pcap_handle_) : nullptr;
            if (detail != nullptr && *detail != '\0') error += ": " + std::string{detail};
            return RawMmsReceiveResult::failure;
        }
        if (header->caplen < 14U || header->caplen > 65'535U) {
            error = "MMS Npcap capture returned an invalid frame length.";
            return RawMmsReceiveResult::failure;
        }
...
private:
    ...
    void move_from(RawMmsPacketReceiver& other) noexcept {
#if defined(__linux__)
