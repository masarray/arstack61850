...
[[nodiscard]] std::span<const std::string_view> semantic_child_order(
    ...
    // OMICRON IEDScout exposes the control-service alternatives in this exact
    // order at LN$CO$DO. Keep deeper Oper/SBOw structures positional from
...
[[nodiscard]] SyntheticChildren collect_synthetic_children(
    ...
    std::sort(begin, end, [&](const SyntheticChild& left, const SyntheticChild& right) noexcept {
        ...
        if (declaration_order) {
            // The real IEDScout capture differs from raw SCL declaration order
