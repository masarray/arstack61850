...
[[nodiscard]] MmsStaticDispatchResult dispatch_attributes(
    ...
    // Exact non-LN objects already carry the authoritative positional MMS
    // TypeSpecification.  This is critical for IEC 61850 FCD/FCDA structures:
    // Read returns the exact object's encoded Data, so GVAA must describe that
    // Data in the identical component order.  Re-synthesizing from flattened
    // descendants would sort ordinary child names and can turn {stVal,q,t}
    // into {q,stVal,t}.
    //
    // Logical Node roots are the exception.  Their static exact object can
    // predate per-association RP/BR/SG/CO objects, so those roots still need
    // final-table synthesis below.
    if (exact != nullptr &&
