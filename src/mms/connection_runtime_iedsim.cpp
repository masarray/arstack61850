...
[[nodiscard]] MmsStaticConnectionResult wrap_mms_response(
    ...
    const std::uint32_t negotiated_mms_pdu_size,
    const std::size_t negotiated_tpdu_size_bytes,
    ...
    if (!encoded_mms.success()) {
        if (encoded_mms.status == wire::EncodeStatus::buffer_too_small &&
            ...
            if (negotiated_mms_pdu_size == 0U ||
                encoded_mms.required_bytes > negotiated_mms_pdu_size) {
                return make_result(
    ...
    if (negotiated_mms_pdu_size == 0U ||
        encoded_mms.bytes_written > negotiated_mms_pdu_size) {
        return make_result(
            MmsStaticConnectionStatus::peer_limit_exceeded,
            state,
            consumed);
...
MmsStaticConnectionResult MmsStaticConnectionRuntime::process_tcp_window(
    ...
    const auto select_workspace_reassembly =
        [this, workspace_storage, &workspace, &reassembly]() noexcept {
    ...
    if (cotp_reassembly_complete_) {
        ...
        if (reassembly.empty()) {
            if (!policy_.require_end_of_transmission) {
                // Explicit compatibility opt-out: expose a partial TPDU to the
    ...
    const auto finish = [this, using_reassembly](
        MmsStaticConnectionResult result) noexcept {
    ...
    if (confirmed_decoded) {
        if (pdv.single_asn1_type.size() > negotiated_mms_pdu_size_) {
    ...
    if (!application.success()) {
        if (application.status == MmsStaticDispatchStatus::response_buffer_too_small) {
            if (negotiated_mms_pdu_size_ == 0U ||
                application.required_bytes > negotiated_mms_pdu_size_) {
                return finish(make_result(
                    MmsStaticConnectionStatus::peer_limit_exceeded,
                    state_,
                    peek.frame_bytes));
