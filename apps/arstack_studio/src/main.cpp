...
int main(int argc, char* argv[]) {
    ...
    if (lifecycleCheck) {
        ...
        QTimer::singleShot(600, mainWindow, [mainWindow] {
            // GitHub-hosted Windows runners do not provide a normal interactive
            // desktop session, so QWindow::close() may not synthesize the same
            // native close path as a user's title-bar X. Post the authoritative
            // Qt close event directly; the production event filter sees exactly
            // the same QEvent::Close generated by a real window close request.
            QCoreApplication::postEvent(mainWindow, new QCloseEvent());
        });
