...
# Complete-helper regression: the browser authoring helper must be intact.
...
if helper_start < 0 or helper_end < 0:
    raise SystemExit("P6C helper boundary missing")
helper = controller[helper_start:helper_end]
for token in ("candidate.logical_node =", "candidate.raw_mms_name =", "return candidate;",
              "QVariantList markOwnedDataSets(", "return dataSets;"):
    require(helper, token, "Complete DataSet helper")
if controller.count("MmsReportController::MmsReportController(") != 1:
    raise SystemExit("P6C controller duplicated")
for forbidden in (
